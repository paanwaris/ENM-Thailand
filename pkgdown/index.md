...
A hands-on workshop on **ecological niche modeling (ENM)** in R, using Sambar deer (*Rusa unicolor*) data as a case study. The workshop walks participants through three R packages that cover the fundamental theory and an applied predictive workflow. The material is organized as four self-contained **Steps**.
...
- [**bean**](https://github.com/paanwaris/bean) *(Step 2)* — reduces **environmental sampling bias** by thinning occurrence points in environmental space (not just geographic space), and fits ellipsoid niches on the thinned points.
